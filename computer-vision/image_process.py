...
def board_array(filename, state):
    ...
    img_size = 800 #size of final transformed board
    ...
    g_pieces = board[:, :, 1] #green pieces, represent crowned pieces

    cv2.imshow('blue', b_pieces)

    piece_list = [b_pieces, r_pieces, g_pieces]
    ...
    board_pts1 = np.float32([[155, 103], [433,98], [148,381], [437, 388]])
    board_pts2 = np.float32([[0, 0], [img_size, 0], [0, img_size], [img_size, img_size]])
    ...
    for row in range(8):
        ...
        for col in range(8):
            ...
            if ((row + col) % 2 == 0) and (np.mean(roi) > config.red_mean_min):
                state[row][col] = 10
            roi = g_pieces[a:b, c:d]
            if ((row + col) % 2 == 0) and (np.mean(roi) > config.green_mean_min):
                state[row][col] = 21
...
#a = config.curr_state
#board_array("computer-vision/repeat_test.jpg", a)
